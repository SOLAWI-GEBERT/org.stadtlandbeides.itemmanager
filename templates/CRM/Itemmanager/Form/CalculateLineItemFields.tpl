  ...
  <script type="text/javascript">
    CRM.$(function($) {
      ...
      function parseLocaleNumber(val) {
        if (!val) return 0;
        return parseFloat(val.replace(thousandMarker, '').replace(separator, '.')) || 0;
      }
      ...
      $('#qty, #unit_price, #financial_type_id', $form).change( function() {
        var unit_price = parseLocaleNumber($('#unit_price', $form).val());
        var qty = parseInt($('#qty', $form).val(), 10) || 0;
